...
@Named
@ViewScoped
public class MinhasComprasCompreGanheBean  implements Serializable{
	...
	public MinhasComprasCompreGanheBean(){
		model = new LazyDataModel<Pedido>() {
			...
			public List<Pedido> load(int first, int pageSize,
				...
				setRowCount(pedidoRepository.quantidadeFiltradosPedidoCompreGanhe(filtroPedido));	
				
				return pedidoRepository.listaPedidoCompreGanhe(filtroPedido);
			}
		};	
	}
